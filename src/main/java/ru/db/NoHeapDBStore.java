...
public class NoHeapDBStore implements DataStore {
    private static final Logger log = LoggerFactory.getLogger(NoHeapDBStore.class);
    private static final String JOURNAL_VERSION = "JAVAOFFHEAPVERSION_1";
    ...
    // the empty records
    //
    private Map<Integer, LinkedList<Long>> emptyIdx = new TreeMap<>();
    ...
    protected final boolean createMessageJournalBB() {
        try {
            ...
            log.error("Ошибка создания журнала In-memory", e);
        }
        return false;
    }

    protected final String createJournalFolderName(String folder, String name) {
        String filename = journalFolder + File.separator +
